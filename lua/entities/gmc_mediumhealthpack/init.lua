...
-- Custom function to handle the cooldown state
...
function ENT:Think()
    if self.Cooldown then return end -- Skip functionality if in cooldown
    ...
    for _, entity in pairs(entitiesInSphere) do
        if IsValid(entity) and entity:IsPlayer() && entity:Alive() then
            local healthIncrease = entity:Health() + (entity:GetMaxHealth() / 2)
